...
export async function ContactMap() {
  ...
  return <section className="flex min-h-64 flex-col items-start justify-center gap-4 rounded-xl border border-border bg-card p-8" aria-label="Business location">
    ...
    <a href={`https://www.google.com/maps?q=${encodeURIComponent(profile.latitude && profile.longitude ? `${profile.latitude},${profile.longitude}` : profile.address)}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-border px-4 py-3 font-medium text-primary">Open Google Maps (new tab)<ExternalLink aria-hidden className="h-4 w-4" /></a>
  </section>;
}
