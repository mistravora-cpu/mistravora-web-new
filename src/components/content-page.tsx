...
export function ContentShell({ title, description, children, parent }: { title: string; description?: string; children: React.ReactNode; parent?: { label: string; href: string } }) {
  return <main className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-8"><Breadcrumbs items={[...(parent ? [parent] : []), { label: title }]} /><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">{title}</h1>{description && <p className="mt-4 max-w-3xl text-lg leading-8 text-muted-foreground">{description}</p>}{children}</main>;
}
