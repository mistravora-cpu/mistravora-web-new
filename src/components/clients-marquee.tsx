    ...
    <section aria-labelledby="trusted-companies-heading" className="w-full site-gutter py-16" data-cv="auto">
      ...
      <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {clients.map((client) => {
          ...
          const content = (
            <>
              <span className="flex h-16 w-16 shrink-0 items-center justify-center">
                {client.logo ? (
                  <Image src={client.logo} alt="" width={64} height={64} sizes="64px" className="h-16 w-16 object-contain" />
                ) : (
          ...
          const className = "flex h-full items-center gap-4 rounded-xl border border-border bg-card/50 p-5";
          return (
            <li key={client.id}>
              {link ? (
