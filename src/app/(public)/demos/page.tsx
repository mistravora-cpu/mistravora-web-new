...
export const metadata = {title:"Demo applications",description:"Explore published Mistravora application demos."};
export default async function DemosPage() {
 ...
 return <ContentShell title="Demo applications" description="Explore the published application previews."><div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{demos.map(demo=><article key={demo.id} className="rounded-xl border border-border p-6">
  ...
  )}<h2 className="text-xl font-semibold">{demo.name}</h2><p className="mt-2 text-muted-foreground">{demo.industry}</p><p className="mt-3">{demo.description}</p><ul className="my-4 list-inside list-disc">{demo.features.map((feature,i)=><li key={i}>{feature}</li>)}</ul>{demo.url && /^https:\/\//.test(demo.url) && <a href={demo.url} target="_blank" rel="noopener noreferrer" className="underline text-primary">Open {demo.name} demo (new tab)</a>}
