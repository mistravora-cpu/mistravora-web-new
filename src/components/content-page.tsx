...
export function ContentShell({ title, description, children, parent }: { title: string; description?: string; children: React.ReactNode; parent?: { label: string; href: string } }) {
  return <main className="mx-auto w-full site-gutter py-16"><Breadcrumbs items={[...(parent ? [parent] : []), { label: title }]} /><h1 className="mt-6 text-3xl font-bold tracking-tight sm:text-4xl">{title}</h1>{description && <p className="mt-4 w-full text-lg leading-8 text-muted-foreground">{description}</p>}{children}</main>;
}
