...
test('robot resumes after stalled preparation and respects context loss and cleanup',async()=>{
 ...
 vm.runInNewContext(ts.transpileModule(code,{compilerOptions:{target:ts.ScriptTarget.ES2022}}).outputText,{
  ...
  useEffect:fn=>{cleanup=fn();},useThree:()=>({gl:{domElement:canvas,extensions:{has:()=>true},compileAsync:()=>new Promise(()=>{})},scene:{},camera:{},pointer:{set:(...point)=>resets.push(point)},events:{connected:canvas},setFrameloop:mode=>frames.push(mode),invalidate:()=>{}}),
 });
