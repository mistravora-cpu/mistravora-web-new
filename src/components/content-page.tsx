...
export function ContentGrid({ entries, prefix }: { entries: ContentEntry[]; prefix: string }) {
  return entries.length ? <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{entries.map(entry => <article key={entry.slug} className="rounded-xl border border-border bg-card p-6">{entry.image && (
    /* eslint-disable-next-line @next/next/no-img-element */
    <img src={entry.image} alt={entry.title} loading="lazy" decoding="async" width={800} height={450} className="mb-4 aspect-video w-full rounded-lg object-cover" />
