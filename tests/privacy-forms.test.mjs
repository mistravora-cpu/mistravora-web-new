...
function load(file, newsletterEnabled = "true"){const context={exports:{},URL,require:(name)=>{
...
context.require=(name)=>name==='@/lib/business-profile'?{getBusinessProfile:async()=>({newsletterEnabled})}:name==='@/lib/supabase/server'?{createClient:()=>{throw Error('Database accessed');}}:name==='@/lib/supabase/admin'?{createAdminClient:()=>{throw Error('Database accessed');}}:name==='@/lib/rate-limit'?{checkRateLimit:()=>null,RATE_LIMITS:{contact:{},newsletter:{}}}:name.startsWith('./')?load('src/lib/'+name.slice(2)+'.ts'):require(name);
...
test('disabled newsletter rejects direct submissions before writing a subscriber', async () => {
 const api = load('src/app/api/newsletter/route.ts', 'false');
 const response = await api.POST(new Request('https://example.com/api/newsletter', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email:'test@example.com',privacyConsent:'yes'}) }));
